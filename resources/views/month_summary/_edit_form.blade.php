  ...
  <div class="id_change">
    ...
    @if ($ms_slip->is_cash === 0)
      <div class="is_cash_value">
        <div class="c-row">現金</div>
      </div>
    @else
      <div class="is_cash_value">
        <div class="c-row">クレジット</div>
      </div>
    @endif
 
    <div class="is_cash_change">
      <input type="hidden" name="is_cash" id="is_cash" value="0">
      <div class="c-row"><input type="checkbox" name="is_cash" class="form-check-input" id="is_cash" value="1" @if ($ms_slip->is_cash == 1) checked @endif>クレジット</div>
    </div>

    <div class="subject_name_value">
      <div class="c-row">{{ $ms_slip->subject_name }}</div>
    </div>
    ...
    <div class="accrual_year_value">
      <div class="c-row">{{ $ms_slip->accrual_year }}/{{ $ms_slip->accrual_month }}/{{ $ms_slip->accrual_date }}</div>
    </div>
  ...
  <!-- マイナスは赤字にする -->
    @if ($ms_slip->price > 0)
      <div class="price_value">
        <div class="c-row">¥{{ number_format($ms_slip->price) }}</div>
      </div>
    ...
    @if ($ms_slip->subtotal > 0)
      <div class="subtotal_value">
        <div class="c-row">¥{{ number_format($ms_slip->subtotal) }}</div>
      </div>
    ...
    @endif
      ...
      <div class="sales_tax_rate_value">
        <div class="c-row">{{ $ms_slip->sales_tax_rate }}%</div>
      </div>
      <div class="sales_tax_rate_change">
        <input type="number" class="c-row ajast-narrow" name="sales_tax_rate" value="{{ $ms_slip->sales_tax_rate }}">%
      </div>
    ...
    @else
      <div class="canceled sales_tax_value">
        <div class="c-row">¥{{ number_format($ms_slip->sales_tax) }}</div>
      </div>
    ...
    @if ($ms_slip->grand_total > 0)
      <div class="grand_total_value">
        <div class="c-row">¥{{ number_format($ms_slip->grand_total) }}</div>
      </div>
    ...
    <div class="remarks_value">
      <div class="c-row">{{ $ms_slip->remarks }}</div>
    </div>
    ...
    <div class="c-row"><input type="button" value="編集" class="btn btn-info btn-sm edit-line"></div>
    <div class="c-row"><input type="submit" value="保存" class="btn btn-info btn-sm save-line"></div>
    <div class="c-row"><input type="button" value="×" class="btn btn-secondary btn-sm cancel-line"></div>
    <div class="c-row"><input type="button" class="btn btn-outline-danger btn-sm" value="削除"></div>
  @endif
</form>
<hr>
